...
(setq default-tab-width 4) ;; four space 
(setq c-default-style "linux") ;; braces {}
...
(defun c++-build-project()
  (interactive)
  (compile (format "cd .. && make")))
(global-set-key (kbd "<f2>") 'c++-build-project)
...
(defun c++-oj-build()
  (interactive)
  (compile (format "clang++ -g -DDEBUG -std=c++11 -fsanitize=address,bounds,unsigned-integer-overflow,signed-integer-overflow,undefined -fno-omit-frame-pointer %s && for d in *.in ; do time ./a.out < $d ; done" (buffer-file-name))))
(global-set-key (kbd "<f3>") 'c++-oj-build)
